# packages
import pandas as pd
import numpy as np
import glob
import os

# helper
def is_int_like_colname(colname):
    try:
        num = int(colname)
        return 0 <= num <= 99
    except ValueError:
        return False


np.random.seed(13)   # seed for reproducible randomness
#task_names = ["AUDIT"]     # for testing
path = "LLM_data"          # folder with original CSVs
out_path = "simulation_non_random" 
os.makedirs(out_path, exist_ok=True)

# TASK NAMES
task_names = ["AUDIT", "BARRAT", "BART", "CARE", "CCT", "DAST", "DFD", "DFE", 
              "DM", "DOSPERT", "FTND", "GABS", "LOT", "MPL", "PG", "PRI", 
              "SOEP", "SSSV"]

# Collect all unique model names from filenames
all_model_names = set()
for task in task_names:
    pattern = os.path.join(path, f"*_{task}_prompting_results.csv")
    for file in glob.glob(pattern):
        # Remove task + suffix to get model name
        model_name = os.path.basename(file).replace(f"_{task}_prompting_results.csv", "")
        all_model_names.add(model_name)

all_model_names = list(all_model_names)

num_models = len(all_model_names)

# Random offsets per model for systematic differences
np.random.seed(13)  # reproducible offsets
model_offsets = {m: np.random.uniform(-2, 2) for m in all_model_names}

# Simulation loop
for task in task_names:
    pattern = os.path.join(path, f"*_{task}_prompting_results.csv")
    for file in glob.glob(pattern):
        model_name = os.path.basename(file).replace(f"_{task}_prompting_results.csv", "")
        
        df = pd.read_csv(file)
        
        # Detect columns to randomize
        logprob_cols = [c for c in df.columns if c.startswith("log_prob_")]
        intlike_cols = [c for c in df.columns if is_int_like_colname(c)]
        cols_to_randomize = list(set(logprob_cols + intlike_cols))
        
        df_work_with = df.copy()

        # Get model-specific offset (0 if unknown)
        offset = model_offsets.get(model_name, 0.0)

        # Randomize selected columns with normal distribution + model offset
        for c in cols_to_randomize:
            df_work_with[c] = np.random.normal(loc=-10 + offset, scale=5, size=len(df_work_with))

        # Save output
        out_file = os.path.join(out_path, f"{model_name}_{task}_prompting_results.csv")
        df_work_with.to_csv(out_file, index=False)

        print(f"Saved systematic randomized file: {out_file}")
