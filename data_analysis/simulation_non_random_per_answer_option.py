...
#     # Offsets per model (if semi-random mode is requested)
#     if semi_random:
#         model_offsets = {m: np.random.uniform(-2, 2) for m in all_model_names}
#     else:
#         model_offsets = {m: 0.0 for m in all_model_names}

#     # If multi-option mode is on: prepare option-specific means per model
#     # (we can't know K upfront, so we compute means later when K is known)
#     option_means_per_model = {}
...
#                 # create model-specific means if not already created
#                 if model not in option_means_per_model:
#                     option_means_per_model[model] = np.random.normal(
#                         loc=mean + offset,
#                         scale=sd,
#                         size=K
...
#                 # for each option column, draw from its own N(mean_k, sd)
#                 for k, col in enumerate(cols_to_randomize):
#                     df_sim[col] = np.random.normal(
#                         loc=option_means[k],
#                         scale=sd,
...
#             else:
#                 # ORIGINAL MODE:
#                 # one distribution per model, possibly with offset
#                 for col in cols_to_randomize:
#                     df_sim[col] = np.random.normal(
#                         loc=mean + offset,
#                         scale=sd,
...
#     parser.add_argument("--multi_option", type=lambda x: x.lower() == "true",
#                         default=False,
#                         help="Use one distribution per answer option per model (True/False).")
...
# #python3 simulate.py --multi_option True
# #python3 simulate.py --semi_random True --multi_option True